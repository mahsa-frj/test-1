...
function install_dependencies {
    apt-get update
    apt-get install -y -qq wget openjdk-8-jre bridge-utils net-tools bsdmainutils unzip traceroute
}
...
function install_vfw_scripts {
    ...
    wget "https://raw.githubusercontent.com/mahsa-frj/test-1/master/heat/vFW_CNF_CDS/templates/helm/vpkg/tap.sh"
    mv tap.sh v_packetgen_init.sh
    chmod +x *.sh
    ...
    sed -i 's|/opt/honeycomb/sample-distribution-1.6.0/honeycomb|/opt/honeycomb/honeycomb|g' v_packetgen_init.sh
